...
export async function POST() {
  ...
  try {
    const response = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:chatId,text:'✅ OzarEye est connecté. Les alertes contiendront toujours la source, l’heure, les coordonnées et le score de corroboration.'}),signal:AbortSignal.timeout(10_000)});
