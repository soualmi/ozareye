...
notes.push(`2. **Contexte de regroupement.** Le live utilise \`activeEvents(24)\` (24 h avant *maintenant*) ; en replay cela ne renverrait rien pour août, donc le contexte vient de \`eventsBetween()\` ancré sur le jour rejoué (J-24 h → fin de J).`);
notes.push('3. **Garde-fou « source permanente » (30 jours) non appliqué.** Il exige plus de 10 jours distincts d\'historique par cellule ; une fenêtre de replay de quelques jours ne peut pas en produire. Il n\'a donc rien supprimé, et surtout : **les torchères, cimenteries et autres sources industrielles permanentes que le live aurait filtrées sont présentes ici**. La détection d\'occupation du sol (Overpass) reste le seul filtre industriel actif dans ce rejeu.');
