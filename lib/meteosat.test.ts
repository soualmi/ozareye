...
test('rule d: a later VIIRS pass re-anchors a Meteosat-only event onto the VIIRS position', () => {
  const [event] = clusterDetections([meteosatDet({ latitude: 36.500, longitude: 5.500 })], []);
  assert.equal(event.positionSource, 'meteosat');

  // ~2.4km away: inside the widened 3km re-anchor radius (rule d), outside
  // the ordinary 2km VIIRS-VIIRS join radius — proving it's the widened
  // radius, not the normal one, that makes this match.
  const [reanchored] = clusterDetections([viirsDet({ latitude: 36.522, longitude: 5.500, acquiredAt: '2026-01-01T00:10:00Z', frp: 40 })], [event]);

  assert.equal(reanchored.positionSource, 'viirs');
